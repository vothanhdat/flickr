...
@withLazyLoad({
  Parallax: () => import(/* webpackChunkName: "spring" */'react-spring').then(e => e.Parallax),
})
class Paralax extends React.Component {
  render() {
    const { classes, Parallax } = this.props
    Parallax.Layer = Parallax.Layer || Parallax
    return (
      <div style={{ background: '#253237', position: "fixed" }} className={classes.absolute}>
        <Parallax ref={ref => (this.parallax = ref)} pages={3} config={{ timing: 1 }}>
...
export default class extends React.Component {
  render() {
    ...
    return <div className={classes.root} >
      <div className={classes.container} style={{ minHeight: "20vw" }}>
        <Paralax classes={classes}></Paralax>
      </div>
    </div>
  }
}